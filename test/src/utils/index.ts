...
export function getFactoryAddress(chainId: number, isV2: boolean) {
  if (isV2) {
    return getChainConfig(chainId).factoryV2Address;
  }
  return getChainConfig(chainId).factoryV3Address;
}
...
export function tokenAmountToDecimal(tokenAmount: BigInt, exchangeDecimals: number): BigDecimal {
  if (exchangeDecimals == 0) {
    return BigDecimal(tokenAmount.toString());
  }
  return BigDecimal(tokenAmount.toString()).div(exponentToBigDecimal(exchangeDecimals));
}

export function priceToDecimal(amount: BigDecimal, exchangeDecimals: number): BigDecimal {
  if (exchangeDecimals == 0) {
    return amount;
  }
